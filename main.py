...
async def main(page: ft.Page):
    ...
    async def fetch_user_data():
        async with aiohttp.ClientSession() as session:
            async with session.get(f'{Config.get_config(0, "config_app").API_URL}/get-data', headers={'Content-Type': 'application/json'}) as response:
                try:
    ...
    async def router(route: str) -> None:
        page.views.clear()
        if page.route.startswith("/start"):
            args = page.route.split("?")
            args = args[1].split("&")
            args = [arg[1].split("=")[1] for arg in args]
            await page.go_async("/boobs")
            user_data = args
            if user_data[0]:
...
if __name__ == '__main__':
    try:
        text = 'App started'
        print(f'{text:*^30}')
        ft.app(target=main, view=None, port=8000, host="127.0.0.1")
    except:
